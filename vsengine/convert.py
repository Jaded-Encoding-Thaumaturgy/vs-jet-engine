...
from vsengine._helpers import EnvironmentTypes, use_inline, wrap_variable_size


# The heuristics code for nodes.
# Usually the nodes are tagged so this heuristics code is not required.
@functools.lru_cache
def yuv_heuristic(width: int, height: int) -> Mapping[str, str]:
    result = {}
...
def _convert_yuv(
    ...
    *,
    ...
    scaler: str | Callable[..., vs.VideoNode],
) -> vs.VideoNode:
    ...
    args = {**yuv_heuristic(c.width, c.height), **default_args}
    ...
    return resizer(
        c, format=real_rgb24.replace(sample_type=c.format.sample_type, bits_per_sample=c.format.bits_per_sample), **args
    )


# Move this function out of the closure to avoid capturing clip.
def _actually_resize(
    c: vs.VideoNode, *, core: vs.Core, convert_yuv: Callable[[vs.VideoNode], vs.VideoNode], target_rgb: vs.VideoFormat
) -> vs.VideoNode:
    ...
    if (
        c.format.color_family != vs.RGB
        or c.format.sample_type != vs.INTEGER
        or c.format.bits_per_sample != target_rgb.bits_per_sample
    ):
...
def to_rgb(
    ...
    *,
    ...
    # Input: YUV
    scaler: str | Callable[..., vs.VideoNode] = "Bicubic",
    default_matrix: str | None = None,
    default_transfer: str | None = None,
    default_primaries: str | None = None,
    default_range: str | None = None,
    default_chromaloc: str | None = None,
) -> vs.VideoNode:
    """
    ...
    :param yuv_heuristic: The heuristic function that takes the frame size and returns a set of yuv-metadata.
                        (For test purposes)
    """
    ...
    with use_inline("to_rgb", env):
        ...
        convert_yuv = functools.partial(
            _convert_yuv, core=core, real_rgb24=real_rgb24, default_args=default_args, scaler=scaler
        )

        actually_resize = functools.partial(_actually_resize, core=core, target_rgb=target_rgb, convert_yuv=convert_yuv)

        return wrap_variable_size(clip, force_assumed_format=target_rgb, func=actually_resize)
